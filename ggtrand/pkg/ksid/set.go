...
func (set CompressedSet) GoString() string {
	b := bytes.Buffer{}
	b.WriteString("ksid.CompressedSet{")
	set.writeTo(&b)
	b.WriteByte('}')
	return b.String()
}
...
func AppendCompressed(set []byte, ids ...KSID) CompressedSet {
	if len(ids) != 0 {
		...
		// The first KSID is always written to the set, this is the starting
		// point for all deltas.
		set = append(set, byte(rawKSUID))
		set = append(set, ids[0].Data[:]...)
		...
		for i := 1; i != len(ids); i++ {
			...
			if t != timestamp {
				...
				set = append(set, id.Data[TimestampSize:]...)
...
func rangeLength(ids []KSID, timestamp uint32, lastKSUID KSID, lastValue uint128) (length int, count int) {
	one := makeUint128(0, 1)
...
// Next moves the iterator forward, returning true if there a KSID was found,
...
func (it *CompressedSetIter) Next() bool {
	...
	case rawKSUID:
		...
		copy(it.KSUID.Data[:], it.content[off0:off1])
	...
	case timeDelta:
		off0 := it.offset
		off1 := off0 + cnt
		off2 := off1 + PayloadSize

		it.timestamp += varint32(it.content[off0:off1])

		binary.BigEndian.PutUint32(it.KSUID.Data[:TimestampSize], it.timestamp)
		copy(it.KSUID.Data[TimestampSize:], it.content[off1:off2])
	...
	default:
		panic("KSID set iterator is reading malformed data")
	}
