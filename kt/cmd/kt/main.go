...
1. 通用设置 brokers 和 topic
    1. 通过环境变量: export KT_BROKERS=192.168.1.1:9092,192.168.1.2:9092,192.168.1.3:9092 KT_TOPIC=topic.test KT_VERSION=0.10.0.0 KT_AUTH=user:123123
    2. 通过命令参数: kt -b=192.168.1.1:9092,192.168.1.2:9092,192.168.1.3:9092 -t elastic.backup -v 0.10.0.0 consume，不方便的是，导致命令过长，每次执行，都得带上这两个参数
2. 消费最新消息: kt consume (环境变量 CLEAN_MSG=1 清除转移符号）
3. 生产消息
    1. 直接消息：echo '你要发送的消息载荷' |  kt produce -t greetings
    2. 指定 key 和 partition : echo '{"key": "id-23", "value": "消息载荷", "partition": 0}' | kt produce --json -t greetings
    3. 使用 JJ 命令生成随机消息：N=3 jj -gu a=@姓名 b=@汉字 c=@性别 d=@地址 e=@手机 f=@身份证 g=@发证机关 h=@邮箱 i=@银行卡 j=@name k=@ksuid l=@objectId m='@random(男,女)' n='@random_int(20-60)' o='@random_time(yyyy-MM-dd)' p=@random_bool q='@regex([a-z]{5}@xyz[.]cn)' |  kt produce --topic greetings
