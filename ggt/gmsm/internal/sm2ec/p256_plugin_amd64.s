...
// 1.p256Sqr
// 2.p256OrdSqr
// 3.sm2P256MulInternalEmmansun
// 4.sm2P256SqrInternalEmmansun
// The most affected one is sm2P256MulInternalEmmansun, it uses SIMD register X0 as temp storage.
...
TEXT sm2P256MulInternalEmmansun(SB),NOSPLIT,$8
	CMPB ·supportBMI2+0(SB), $0x01
	JEQ  internalMulBMI2
...
TEXT sm2P256SqrInternalEmmansun(SB),NOSPLIT,$8
	CMPB ·supportBMI2+0(SB), $0x01
	JEQ  internalSqrBMI2
...
#define p256PointAddAffineInline() \
	...
	\
	LDacc (z1sqr)                     \
	CALL sm2P256MulInternalEmmansun(SB)	      \// z1ˆ3
	\
	LDt (y2in)                        \
	...
	\
	LDt (y1in)                        \
	CALL sm2P256MulInternalEmmansun(SB)	      \// s2 = y1 * hˆ3
	ST (s2)                           \
	\
	LDacc (x1in)                      \
	LDt (hsqr)                        \
	CALL sm2P256MulInternalEmmansun(SB)	      \// x1 * hˆ2
	ST (h)                            \
	\
...
#define p256PointAddInline() \
	\// Begin point add
	LDacc (z2in)                 \
	CALL sm2P256SqrInternalEmmansun(SB)	 \// z2ˆ2
	ST (z2sqr)                   \
	LDt (z2in)                   \
	CALL sm2P256MulInternalEmmansun(SB)	 \// z2ˆ3
	LDt (y1in)                   \
...
#define calZ() \
	...
	LDacc (z)                               \
	LDt (y)                                 \
	CALL sm2P256MulInternalEmmansun(SB)             \ // Z1 * Y1
...
#define calX() \
	...
	LDt (m)                                 \
	CALL sm2P256MulInternalEmmansun(SB)             \ // M = (X1 - ZZ) * (X1 + ZZ) = X1^2 - ZZ^2
	ST (m)                                  \
	\// Multiply by 3
	p256TripleInline                        \
	STt (m)                                 \  // M = 3 * (X1^2 - ZZ^2)
	...
	LDacc (y)                               \
	p256MulBy2Inline2                       \
	CALL sm2P256SqrInternalEmmansun(SB)             \ // 4 * YY = (2*Y1)^2
	ST (s)                                  \ // S = 4 * YY
	CALL sm2P256SqrInternalEmmansun(SB)             \ // (4 * YY)^2 = 16 * YYYY
	\// Divide by 2
	XORQ mul0, mul0                         \
	MOVQ acc4, t0                           \
	MOVQ acc5, t1                           \  
	...
	LDacc (x)                               \
	LDt (s)                                 \
	CALL sm2P256MulInternalEmmansun(SB)             \ // X1 * 4 * YY
	ST (s)                                  \ // S = 4 * X1 * YY = 2 * ((X1+YY)^2 - XX - YYYY)
	p256MulBy2Inline                        \
	STt (tmp)                               \ // tmp = 2*S = 8 * X1 * YY
	\
	LDacc (m)                               \
	CALL sm2P256SqrInternalEmmansun(SB)             \ // M^2 = (3 * (X1^2 - ZZ^2))^2
	LDt (tmp)                               \
...
#define calY() \
	...
	\
	LDt (m)                                 \
	CALL sm2P256MulInternalEmmansun(SB)             \ // M * (S - X3)
	\
	LDt (y)                                 \
