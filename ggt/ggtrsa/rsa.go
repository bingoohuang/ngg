...
func main() {
	...
	root.CreateCmd(c, "newkey", "生成公钥私钥", &newKeyCmd{})
	root.CreateCmd(c, "sign", "签名", &signCmd{})
	root.CreateCmd(c, "verify", "验签", &verifyCmd{})
	root.CreateCmd(c, "encrypt", "公钥加密/私钥解密, 私钥加密/公钥解密", &encryptCmd{})
	root.CreateCmd(c, "keypair", "公检测私钥公钥是否匹配", &checkKeyPairCmd{})
...
func (f *encryptCmd) Run(cmd *cobra.Command, args []string) error {
	...
	if f.Pub != "" {
		...
		if err := ggtencrypt.WriteDataFile(action, f.Out, obj.ToBytes(), !f.Decrypt); err != nil {
			return err
		}
	} else if f.Pri != "" {
		action := ""
...
func (r *newKeyCmd) Run(cmd *cobra.Command, args []string) error {
	...
	ggtencrypt.WriteKeyFile(pri.ToKeyBytes(), r.Dir, "rsa_pri.pem")
	...
	// CreateXMLPublicKey().
	ggtencrypt.WriteKeyFile(pub.ToKeyBytes(), r.Dir, "rsa_pub.pem")
