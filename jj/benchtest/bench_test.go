...
func BenchmarkGJSONGet(t *testing.B) {
	...
	for i := 0; i < t.N; i++ {
		for j := 0; j < len(benchPaths); j++ {
			if jj.Get(exampleJSON, benchPaths[j]).Type == jj.Null {
...
func benchmarkGJSONGetManyN(t *testing.B, n int) {
	...
	t.ResetTimer()
	for i := 0; i < t.N; i++ {
		results := jj.GetMany(exampleJSON, paths...)
		if len(results) == 0 {
			t.Fatal("did not find the value")
		}
		for j := 0; j < len(results); j++ {
			if results[j].Type == jj.Null {
				t.Fatal("did not find the value")
			}
		}
	}
...
func BenchmarkGJSONUnmarshalMap(t *testing.B) {
	...
	for i := 0; i < t.N; i++ {
		for j := 0; j < len(benchPaths); j++ {
			...
			m, _ := jj.Parse(exampleJSON).Value().(map[string]any)
			var v any
			for len(parts) > 0 {
				part := parts[0]
				if len(parts) > 1 {
...
func BenchmarkConvertNone(t *testing.B) {
	...
	t.ResetTimer()
	for i := 0; i < t.N; i++ {
		jj.Get(mj, "50.widget.text.onMouseUp")
	}
}
...
func BenchmarkConvertGet(t *testing.B) {
	...
	t.ResetTimer()
	for i := 0; i < t.N; i++ {
		jj.Get(string(data), "50.widget.text.onMouseUp")
	}
}
...
func BenchmarkConvertGetBytes(t *testing.B) {
	...
	t.ResetTimer()
	for i := 0; i < t.N; i++ {
		jj.GetBytes(data, "50.widget.text.onMouseUp")
	}
}
